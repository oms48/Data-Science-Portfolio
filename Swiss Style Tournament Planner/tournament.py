...
def swissPairings():
    ...
    orderedPlayers = cursor.fetchall()
    pairs = [(orderedPlayers[2*i][0], orderedPlayers[2*i][1], orderedPlayers[2*i + 1][0]
              , orderedPlayers[2*i + 1][1]) for i in range(len(orderedPlayers) / 2)]
    connection.close()
    return pairs
